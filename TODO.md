...
- [ ] `P2-07` 实现受信任的动态 Application 模块：`App` 通过显式绝对路径加载一个共享库，并借用其 C 链接、`noexcept` 的 `tos_get_module()` 函数返回的函数内静态 `Module` 对象；不做目录发现、ABI 验证、签名、沙箱或热重载。
  - 验收：有效模块的启动、停止和回调可测；相对/缺失文件、缺失符号、空指针、重名和迟注册返回明确 Status；Linux、macOS、Windows CI 原生加载测试全部通过。模块与宿主须同 tos 版本、编译器和 C++ 运行时。
